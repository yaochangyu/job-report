...
**驗收**
- 修改 builder 後執行 `uv run python run_all.py --from <date_from> --to <date_to> --steps report,html` 重新產生 T2 parquet 並更新 output/。
- 預設頁面載入時間明顯下降。
- 不再一次渲染數十萬列。
...
- [ ] `node --check query-definitions.js`
- [ ] `node --check dashboard-renderers.js`
- [ ] `uv run python run_all.py --from <date_from> --to <date_to> --steps report,html`

**目的**
- 確認前端 JS 無語法錯誤。
- 確認 T2 parquet 與 frontend shell 可完整重建。

### 2. 資料正確性驗證

**這一關是資料正確性的主檢查。**

- 直接讀取 `output/dataset/report/...` 的 T2 parquet。
- 以相同日期區間重新聚合一次，驗證畫面使用的 KPI / 圖表口徑。

**重點檢查**
- [ ] `device`：`COUNT(DISTINCT os)`、`COUNT(DISTINCT browser)`
- [ ] `ranking`：跨日 `COUNT(DISTINCT featureId)`、Top 1 feature 與總量
- [ ] `heatmap`：跨日 `COUNT(DISTINCT feature_id)`、Top 1 feature 與總量
- [ ] `navigation`：跨日 distinct `page` 數，不再使用每日加總

**目的**
- 確認修正的是實際資料口徑，不只是畫面文字。

### 3. UI 手動驗證

- 使用指定日期區間開啟對應 dashboard 頁面。
- 檢查畫面是否正確呈現第 2 關驗證出的數值與文案。

**重點檢查**
- [ ] `feature`：身份辨識 KPI 副標文案
- [ ] `device`：OS / 瀏覽器種類 KPI 是否有正確數值
- [ ] `ranking`：功能數量、Top 1、類別圖是否合理
- [ ] `heatmap`：不重複功能數、Top 1 是否合理
- [ ] `navigation`：KPI 文案是否正確，且頁面載入速度是否改善
