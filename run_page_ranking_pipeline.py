...
───────────────────────────
PoC：串接 page-ranking 的 extract → transform → render。
"""
...
from build_page_ranking_t2 import build_page_ranking_t2
from common.t1_reader import resolve_date_window
from extract_raw_events import extract_raw_events
from render_page_ranking_t3 import render_page_ranking_t3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="執行 page-ranking 三層資料管線 PoC")
...
def main() -> None:
    ...
    print(f"[PIPELINE] page-ranking：{date_from} ~ {date_to}")

    print("[STEP] T1 extract")
    extract_raw_events(date_from, date_to, keep_existing=args.keep_existing)

    print("[STEP] T2 transform")
    t2_dir = build_page_ranking_t2(date_from, date_to)
    print(f"  → {t2_dir}")

    print("[STEP] T3 render")
    output_dir = Path(args.output) if args.output else Path(__file__).parent / "output" / "page-ranking"
