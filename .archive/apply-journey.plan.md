...
- [x] **步驟 1：建立 T2 builder `build_apply_journey_t2.py`**
...
- [x] **步驟 2：將新 builder 加入 `run_all.py`**

  在 REPORTS 清單加入 apply-journey，指定 `view_mode = "apply-journey"`。

- [x] **步驟 3：在 `query-definitions.js` 新增 `applyJourneyPlan`**
...
- [x] **步驟 4：在 `dashboard-renderers.js` 新增 `applyJourneyRenderer`**
...
- [x] **步驟 5：更新 `dashboard-renderers.js` 的 `VIEW_PANEL_TEXT` 與 `showPanels`**
