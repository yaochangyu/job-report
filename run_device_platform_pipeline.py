#!/usr/bin/env python3
"""
run_device_platform_pipeline.py
──────────────────────────────
PoC：串接 device-platform 的 extract → transform → render。
"""
...
from build_device_platform_t2 import build_device_platform_t2
from common.t1_reader import resolve_date_window
from extract_raw_events import extract_raw_events
from render_device_platform_t3 import render_device_platform_t3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="執行 device-platform 三層資料管線 PoC")
...
def main() -> None:
    ...
    print(f"[PIPELINE] device-platform：{date_from} ~ {date_to}")

    print("[STEP] T1 extract")
    extract_raw_events(date_from, date_to, keep_existing=args.keep_existing)

    print("[STEP] T2 transform")
    t2_dir = build_device_platform_t2(date_from, date_to)
    print(f"  → {t2_dir}")

    print("[STEP] T3 render")
    output_dir = Path(args.output) if args.output else Path(__file__).parent / "output" / "device-platform"
