#!/usr/bin/env python3
"""
render_traffic_overview_t3.py
────────────────────────────
從 T2 report parquet 讀取資料並產出 traffic-overview HTML。
"""
...
from build_traffic_overview_t2 import (
    BROWSER_FILE,
    DAILY_FILE,
    DEVICE_FILE,
    HOURLY_FILE,
...
from traffic_overview_report import OUTPUT_DIR, generate_html

REPORT_NAME = "traffic-overview"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="從 T2 產出 traffic-overview HTML")
...
def render_traffic_overview_t3(date_from: str, date_to: str, output_dir: Path) -> Path:
    ...
    hourly = _to_dict_list(snapshot_dir / HOURLY_FILE)
    device = {
        "device_type": _to_dict_list(snapshot_dir / DEVICE_FILE),
