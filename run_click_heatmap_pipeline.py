#!/usr/bin/env python3
"""
run_click_heatmap_pipeline.py
────────────────────────────
PoC：串接 click-heatmap 的 extract → transform → render。
"""
...
from build_click_heatmap_t2 import build_click_heatmap_t2
from common.t1_reader import resolve_date_window
from extract_raw_events import extract_raw_events
from render_click_heatmap_t3 import render_click_heatmap_t3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="執行 click-heatmap 三層資料管線 PoC")
...
def main() -> None:
    ...
    print(f"[PIPELINE] click-heatmap：{date_from} ~ {date_to}")
    ...
    t2_dir = build_click_heatmap_t2(date_from, date_to)
    print(f"  → {t2_dir}")

    print("[STEP] T3 render")
    output_dir = Path(args.output) if args.output else Path(__file__).parent / "output" / "click-heatmap"
