...
"""
run_apply_conversion_pipeline.py
───────────────────────────────
PoC：串接 apply-conversion 的 extract → transform → render。
"""
...
from build_apply_conversion_t2 import build_apply_conversion_t2
from common.t1_reader import resolve_date_window
from extract_raw_events import extract_raw_events
from render_apply_conversion_t3 import render_apply_conversion_t3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="執行 apply-conversion 三層資料管線 PoC")
...
def main() -> None:
    ...
    print(f"[PIPELINE] apply-conversion：{date_from} ~ {date_to}")
    ...
    t2_dir = build_apply_conversion_t2(date_from, date_to)
    print(f"  → {t2_dir}")

    print("[STEP] T3 render")
    output_dir = Path(args.output) if args.output else Path(__file__).parent / "output" / "apply-conversion"
